...
scriptContext:SetCallback(function()
    local script = "-- This script was generated by Vanity Hydroxide's RemoteSpy: https://github.com/TheNuggetEater05/Hydroxide\n\nlocal Network = require(game:GetService(\"ReplicatedStorage\").Modules.Shared.Network)\n\n"
    local selectedRemote = selected.remoteLog.Remote.Instance
    local remoteClassName = selectedRemote.ClassName
    local remotePath = "Network"
    local method
    ...
    if #selected.args == 0 then
        ...
        for i = 1, #selectedArgs do
            ...
            script = script .. ("local vanity%s%d = %s\n"):format(variableName, i, v) 
            args = args .. ("vanity%s%d, "):format(variableName, i)
        end
